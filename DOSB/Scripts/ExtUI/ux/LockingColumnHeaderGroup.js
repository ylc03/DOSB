...
Ext.ux.grid.LockingColumnHeaderGroup = Ext.extend(Ext.util.Observable, {
    ...
    viewConfig: {
        ...
        renderHeaders: function(){
			...
			var lgs = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getLockedGroupStyle.call(this);
			var ltstyle = 'width:' + lgs.width + ';';
	
            for(var row = 0, rlen = rows.length; row < rlen; row++){
                var r = rows[row], cells = [];
                
				// unlocked group headers
				for(var i = 0, gcol = 0, len = r.length; i < len; i++){
                    ...
                    var id = this.getColumnId(group.dataIndex ? cm.findColumnIndex(group.dataIndex) : gcol), 
						gs = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getGroupStyle.call(this, group, gcol);
				...
				var lcell = ts.gcell.apply({
                    cls: 'ux-grid-hd-group-cell',
                    id: 'locked',
                    row: row,
                    style: 'width:' + lgs.width + ';' + (lgs.hidden ? 'display:none;' : ''),
					istyle: '',
					btn: this.grid.enableHdMenu && '&nbsp;',
					value: '&nbsp;'
                });
				lockedHeaders[row] = ts.header.apply({
                    tstyle: ltstyle,
                    cells: lcell
                });
            }

            superHeaders = this.constructor.prototype.renderHeaders.apply(this, arguments);
			headers.push(superHeaders[0]);
			lockedHeaders.push(superHeaders[1]);
			superHeaders[0] = headers.join('');
			superHeaders[1] = lockedHeaders.join('');
            return superHeaders;
        },

        onColumnWidthUpdated: function(){
            this.constructor.prototype.onColumnWidthUpdated.apply(this, arguments);
            Ext.ux.grid.LockingColumnHeaderGroup.prototype.updateGroupStyles.call(this);
        },
        ...
        afterRenderUI: function(){
            ...
            Ext.apply(this.columnDrop, Ext.ux.grid.LockingColumnHeaderGroup.prototype.columnDropConfig);
            Ext.apply(this.splitZone, Ext.ux.grid.LockingColumnHeaderGroup.prototype.splitZoneConfig);
        }
    },
    ...
    columnDropConfig: {
        ...
        positionIndicator: function(h, n, e){
            var data = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getDragDropData.call(this, h, n, e);
            if(data === false){
                return false;
            }
        ...
        onNodeDrop: function(n, dd, e, data){
            var h = data.header;
            if(h != n){
                var d = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getDragDropData.call(this, h, n, e);
                if(d === false){
                    return false;
                }
    ...
    getGroupStyle: function(group, gcol){
        ...
        for(var i = gcol, len = gcol + group.colspan; i < len; i++){
            if(!this.cm.isHidden(i) && !this.cm.isLocked(i)){
            ...
            if(row < rows.length){
                ...
                for(var i = 0, gcol = 0; i < cells.length; i++){
                    ...
                    if((typeof col != 'number') || (col >= gcol && col < gcol + group.colspan)){
                        var gs = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getGroupStyle.call(this, group, gcol);
    ...
    getDragDropData: function(h, n, e){
        ...
        var row = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getGroupRowIndex.call(this.view, h),
            oldGroup = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getGroupSpan.call(this.view, row, oldIndex),
            newGroup = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getGroupSpan.call(this.view, row, newIndex),
        ...
        var parentGroup = Ext.ux.grid.LockingColumnHeaderGroup.prototype.getGroupSpan.call(this.view, row - 1, oldIndex);
